...
<script>
	...
	{
		axios.post('<?=base_url();?>'+'config/background_login/data/'+arg+"/"+id)
		.then(function (response)
		{
			if (response.status == 200) 
			{
				if (arg == 'card') {
					var html_card = '<div class="col s12 m6 l4 card-width">'+
										'<div class="card-panel border-radius-6 mt-10 card-animation-1">'+
											'<img class="responsive-img border-radius-8 z-depth-4 image-n-margin" src="<?=base_url().$path;?>33.png" alt="images">'+
											'<h6><a href="#" class="mt-5">Default</a></h6>'+
												// '<p>'+res.photo+'</p>'+
											'<div class="row mt-4">'+
											'</div>'+
										'</div>'+
					...
					_.forEach(response.data.list, function(res,index) 
					{
						html_card += '<div class="col s12 m6 l4 card-width">'+
										'<div class="card-panel border-radius-6 mt-10 card-animation-1">'+
											'<img class="responsive-img border-radius-8 z-depth-4 image-n-margin" src="<?=base_url().$path;?>'+res.image+'" alt="images">'+
											'<h6><a href="#" class="mt-5">'+res.image+'</a></h6>'+
												// '<p>'+res.image+'</p>'+
											'<div class="row mt-4">'+
												'<div class="col s6">'+													
													((data_rules.update == 1) ? loadButton('btn','edit','edit',res.id,res.image,'','default_class') : '') +
												'</div>'+
												'<div class="col s6 right-align">'+
													((data_rules.delete == 1) ? ((res.status == 1) ? '' : loadButton('btn','delete','delete',res.id,res.image,'','default_class')) : '') +
												'</div>'+
												'<div class="col s12">'+
													((data_rules.update == 1) ? ((res.status == 1) ? loadButton('btn','off','off',res.id,res.image,'Matikan','default_class') : loadButton('btn','on','on',res.id,res.image,'Hidupkan','default_class') ) : '') +
												'</div>'+																							
											'</div>'+
											'<div class="row">'+
											'</div>'+
				...
				else if(arg == 'single')
				{
					...
					var html_card = '<div class="col s12 m6 l4 card-width">'+
										'<div class="card-panel border-radius-6 mt-10 card-animation-1">'+
											'<img class="responsive-img border-radius-8 z-depth-4 image-n-margin" src="<?=base_url().$path;?>'+obj.image+'" alt="images">'+
										'</div>'+
									'</div>';
	...
	function send_file(param,id){
		...
		axios.post('<?=base_url();?>'+'config/background_login/upload_file/'+param+'/'+id, data)			
		.then(function (response) {
			if (response.status == 200) 
			{
	...
	function sendData(data_sender) {		
		...
		axios.post('<?=base_url();?>'+'config/background_login/store', data_sender)
		.then(function (response) {
			if (response.status == 200) {
				send_response(response,'report');
				if (response.data.status == true) {					
	...
	function openForm(arg,id,name) {		
		...
		else if (arg == 'on')
		{
			Notiflix.Confirm.Show(
				'Aktifkan',
				'Anda yakin aktifkan data ini sebagai background login ?',
				'Ya, aktifkan',
				'Tidak',
				function(){ 
					var data_sender = {
		...
		else if (arg == 'off')
		{
			Notiflix.Confirm.Show(
				'Matikan',
				'Anda yakin matikan data ini sebagai background login ?',
				'Ya, aktifkan',
				'Tidak',
				function(){ 
					var data_sender = {
