...
<script>
	...
	function loadData(arg,id) 
	{
		...
		axios.post('<?=base_url();?>'+'fondation/salary_sources/data/'+arg+'/'+id)
				...
				if (arg == 'table') 
				{
					...
					_.forEach(response.data.list, function(res,index) 
					{
						state.dom.insert += '<tr>'+
											'<td>'+(index+1)+'</td>'+
											'<td>'+res.name+'</td>'+
											'<td> Rp. '+numberWithCommas(res.value)+'</td>'+											
											'<td>'+
	...
	function sendData(data_sender) {
		state_process('before_send')		
		axios.post('<?=base_url();?>'+'fondation/salary_sources/store', data_sender)
		.then(function (response) {
