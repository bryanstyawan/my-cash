...
class Category_expenses extends CI_Controller
{
	...
	{
		$this->data['title']	= 'Category > Expenses';
		$view			= $this->load->view('fondation/category/expenses',$this->data);
		return $view;
	}	
	...
	public function data($arg,$id)
	{
		...
		if ($arg == 'table') {
			# code...
			$this->data['list'] = $this->Stores->getWhere('fdn_category_expenses',array('AND'=>array('status' => 1)))->result_array();			
		}
		else
		{
			$this->data['list'] = $this->Stores->getWhere('fdn_category_expenses',array('AND'=>array('id'=>$id,'status' => 1)))->result_array();			
		}
